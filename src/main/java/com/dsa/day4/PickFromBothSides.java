...
Explanation 2:
 ...
 Remove the first element and the last 3 elements. So we get 2 + 4 + 2 + 1 = 9
    ...
    public static void main(String[] args) {
        ...
        int query = 4;
        System.out.println("Max count is : "+maxSumFromPickBothSides(nums, query));

    }
    private static int maxSumFromPickBothSides(int[] nums, int query){
        int sum = 0;
        int maxi = 0;
        for(int i=0;i<query;i++)  sum += nums[i];
